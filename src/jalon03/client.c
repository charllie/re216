...
int main(int argc,char** argv)
{
	...
	printf("\nChat Room :\n\nPlease logon with /nick <your pseudo>\n\n-> ");

	//record username and send to the server
	fgets(msg, LENGTH, stdin);
	while( strncmp(msg,"/nick ",6) ){
	...
	recv(sock, msg, LENGTH, 0);
	printf("%s",msg);
	memset(msg, 0, LENGTH);

	while( strncmp(msg, "/quit\n", 7) && strcmp(buffer, "Server stopped working") )
	{
