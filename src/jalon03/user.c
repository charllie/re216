	...
	if (string != NULL)
	{
		...
		if (user_find_by_nickname(l, string)) {
			if (!strncmp(user->nickname, string, PLENGTH)) {
				if (user->logged)
					do_write(user->sock, "This nickname is already yours");
				else {
					...
					char buffer[LENGTH] = "Welcome on the chat ";
				...
				if (user->logged)
					do_write(user->sock, "This nickname is already used");
				else
					do_write(user->sock, "This nickname is already used - Please logon with /nick <your pseudo>");
			}
		//it's OK
		} else {
			strncpy(user->nickname, string, PLENGTH);
		...
		do_write(user->sock, "Please logon with /nick <your pseudo>");
	}
}
...
{
	user * p = list_get_head(l);
	char nickname[LENGTH] = "Online users are";
